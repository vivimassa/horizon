...
import { X, SlidersHorizontal, Loader2, ChevronRight } from 'lucide-react'
import { Dialog, DialogContent, DialogHeader, DialogTitle } from '@/components/ui/dialog'
import { Switch } from '@/components/ui/switch'
import type { SAProgress, SAResult } from '@/lib/utils/ops-tail-assignment-sa'

// MIP types (Cloud Run solver)
export interface MIPProgress { phase: 'building' | 'solving' | 'extracting'; message: string; elapsedMs: number }
...
export type OpsOptimizerMethod = 'ai' | 'optimal'

export interface OpsOptimizerDialogProps {
  open: boolean
  onClose: () => void
  onRunComplete: (method: OpsOptimizerMethod, aiPreset?: 'quick' | 'normal' | 'deep') => void
  currentMethod: OpsOptimizerMethod
...
export function OpsOptimizerDialog({
  open,
  onClose,
  onRunComplete,
  currentMethod,
  ...
  accentColor = '#3B82F6',
  container,
}: OpsOptimizerDialogProps) {
  const router = useRouter()
  const [selectedMethod, setSelectedMethod] = useState<OpsOptimizerMethod>(currentMethod)

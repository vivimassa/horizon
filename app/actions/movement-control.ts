...
export interface MovementRouteData {
  id: string
  ...
  status: string
  notes: string | null
  legs: MovementRouteLeg[]
}

export interface MovementRouteLeg {
  id: string
  legSequence: number
...
export async function getRouteWithLegs(routeId: string): Promise<MovementRouteData | null> {
 ...
 */
export async function getRoutesForFlights(flightIds: string[]): Promise<MovementRouteData[]> {
  if (flightIds.length === 0) return []
  ...
  const results: MovementRouteData[] = []

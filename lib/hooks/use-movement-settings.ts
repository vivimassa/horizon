...
import { getMovementSettings, saveMovementSettings } from '@/app/actions/movement-settings'
...
export type { MovementSettingsData }
export { DEFAULT_MOVEMENT_SETTINGS }

const STORAGE_KEY = 'movement_settings_v1'
const OLD_BAR_COLORS_KEY = 'movement_bar_colors'

type SaveStatus = 'idle' | 'saving' | 'saved'

/** Merge partial stored data with defaults so every key is guaranteed present. */
function mergeWithDefaults(partial: Record<string, unknown>): MovementSettingsData {
  const d = DEFAULT_MOVEMENT_SETTINGS
  ...
  return {
    assignmentMethod: (partial.assignmentMethod as MovementSettingsData['assignmentMethod'] | undefined) ?? d.assignmentMethod,
    timeDisplay: (partial.timeDisplay as MovementSettingsData['timeDisplay'] | undefined) ?? d.timeDisplay,
    ...
    barLabels,
    fleetSortOrder: (partial.fleetSortOrder as MovementSettingsData['fleetSortOrder'] | undefined) ?? d.fleetSortOrder,
...
function loadFromStorage(): MovementSettingsData {
  if (typeof window === 'undefined') return { ...DEFAULT_MOVEMENT_SETTINGS }
  try {
    ...
    if (stored) {
      ...
      if (OLD_PASTEL_COLORS.has(parsed.colorAssignment.unassigned)) {
        parsed.colorAssignment.unassigned = DEFAULT_MOVEMENT_SETTINGS.colorAssignment.unassigned
        migrated = true
      }
      if (OLD_PASTEL_COLORS.has(parsed.colorAssignment.assigned)) {
        parsed.colorAssignment.assigned = DEFAULT_MOVEMENT_SETTINGS.colorAssignment.assigned
        migrated = true
      }
...
function saveToStorage(settings: MovementSettingsData) {
...
function applyCssVars(settings: MovementSettingsData) {
  if (typeof document === 'undefined') return
  const root = document.documentElement
  const isDark = root.classList.contains('dark')
  const colors = settings.colorAssignment ?? DEFAULT_MOVEMENT_SETTINGS.colorAssignment
...
export function useMovementSettings() {
  const [settings, setSettings] = useState<MovementSettingsData>(loadFromStorage)
  ...
  useEffect(() => {
    ...
    getMovementSettings().then(remote => {
  ...
  const debouncedSave = useCallback((next: MovementSettingsData) => {
    if (debounceRef.current) clearTimeout(debounceRef.current)
    debounceRef.current = setTimeout(async () => {
      setSaveStatus('saving')
      await saveMovementSettings(next)
  ...
  const applyUpdate = useCallback((updater: (prev: MovementSettingsData) => MovementSettingsData) => {
    setSettings(prev => {
  ...
  const updateSettings = useCallback((partial: Partial<MovementSettingsData>) => {
    applyUpdate(prev => ({ ...prev, ...partial }))
  }, [applyUpdate])

  const updateDisplay = useCallback((key: keyof MovementSettingsData['display'], value: boolean) => {
    applyUpdate(prev => ({ ...prev, display: { ...prev.display, [key]: value } }))
  }, [applyUpdate])

  const updateColorAssignment = useCallback((colors: Partial<MovementSettingsData['colorAssignment']>) => {
  ...
  const updateTooltip = useCallback((key: keyof MovementSettingsData['tooltip'], value: boolean) => {
  ...
  const resetAll = useCallback(() => {
    const defaults = { ...DEFAULT_MOVEMENT_SETTINGS }
